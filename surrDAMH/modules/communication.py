...
class SurrogateLocal_CollectorMPI(Communicator):
    ...
    def send_to_collector(self, snapshot):
        ...
        if self.tag-TAG_FIRST_SNAPSHOT >= self.conf.max_sampler_isend_requests:
            self.requests[(self.tag-TAG_FIRST_SNAPSHOT) % self.conf.max_sampler_isend_requests].wait()
            self.requests[(self.tag-TAG_FIRST_SNAPSHOT) % self.conf.max_sampler_isend_requests] = request_send
        else:
            self.requests.append(request_send)
        self.tag += 1
        ################################################################
