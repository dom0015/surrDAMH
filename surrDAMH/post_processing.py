...
class Samples:
    ...
    def _plot_hist_1d(self, axis, burn_in=None, param_no=0, stages_to_disp=None, bins=20, show=True, log=False):
        # use weighted data (compressed)
        all_x = np.zeros((0,))
        no_unique_samples_sum = 0
        for j in stages_to_disp:
            for i in range(self.list_of_stages[j].no_chains):
                ...
                try:
                    tmp = self.list_of_stages[j].samples[i][burn_in[j][i]:, param_no]
                except:
    ...
    def _plot_hist_2d(self, axis, burn_in=None, param_no=[0, 1], stages_to_disp=None, bins=20, show=True, colorbar=False, log=[False, False]):
        # use weighted data
        all_x = np.zeros((0,))
        all_y = np.zeros((0,))
        no_unique_samples_sum = 0
        for j in stages_to_disp:
            for i in range(self.list_of_stages[j].no_chains):
                no_unique_samples_sum += self.list_of_stages[j].no_unique_samples[i]  # burn-in not excluded
                try:
                    tmp_x = self.list_of_stages[j].samples[i][burn_in[j][i]:, param_no[0]]
                    tmp_y = self.list_of_stages[j].samples[i][burn_in[j][i]:, param_no[1]]
                except:
    ...
    def plot_hist_grid(self, bins1d: int = 20, bins2d: int = 20,
                       ...
                       parameters_to_disp: list[int] = None, stages_to_disp: list[int] = None):
        ...
        for idi, i in enumerate(parameters_to_disp):
            for idj, j in enumerate(parameters_to_disp):
                axis = axes[idi, idj]
                if idi == idj:
                    self._plot_hist_1d(axis=axis, param_no=i, burn_in=burn_in, stages_to_disp=stages_to_disp, bins=bins1d, show=False, log=log_scale[i])
                else:
