...
C = Configuration()
problem_name = "GRF_24to10_06_unlimited_half"
...
S.remove_chains(range(N,3*N)) # kep only DAMH chains
...
S.plot_evaluated(N,L=S.evaluated,title="evaluated")
S.plot_evaluated(N,L=S.accepted,title="accepted")
S.plot_evaluated(N,L=S.rejected,title="rejected")

S.plot_evaluated_sliding(range(N), no_sw=20, L = S.evaluated, window_length=90000)
S.calculate_autocorr_time_sliding(no_sw=20, chains_range=range(N), window_length=90000)
...
S.plot_evaluated_sliding(range(N,2*N), no_sw=20, L = S.evaluated, window_length=90000)
S.calculate_autocorr_time_sliding(no_sw=20, chains_range=range(N,2*N), window_length=90000)
plt.plot(S.tau_sliding_min)
plt.plot(S.tau_sliding_mean)
plt.plot(S.tau_sliding_max)
plt.title("without update")
plt.show()
