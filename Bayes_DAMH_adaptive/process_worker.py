...
def worker(num,comm_world,Model,SF,Surrogate,no_solvers,initial_samples,group_leader_ids,proposalStd):
...
#                    print("DEBUG:", no_evaluations, SF.shared_finisher.value, queue_size)
                    SOL, no_evaluations, alldata_par, alldata_obs, alldata_wei, TEMP2, RHS = Surrogate.calculate(alldata_par,alldata_obs,alldata_wei,Model.no_parameters,Surrogate.parameters)
...
#                time.sleep(0.1)
    ...
    if num>2:
        ...
        Sampler = MHsampler.MHsampler(proposalStd, num-SF.no_helpers)
