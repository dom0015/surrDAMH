...
class scm(Surrogate_parent.Surrogate_parent):
    def __init__(self):
        print('SCM surrogate model will be constructed.')
        
    def calculate(self,alldata_par, alldata_obs, alldata_wei, no_parameters, scm_parameters):
        alldata_wei = alldata_wei*0 + 1 # TEMP !!!
        dim, max_degree, x1, x2, x3 = scm_parameters
        no_snapshots = alldata_par.shape[0]
        degree = int(np.floor(np.log(no_snapshots)/np.log(dim)))
        print("degree",degree)
        if degree == 0:
            degree=1
        if degree>max_degree:
            degree=max_degree
        poly = self.generate_polynomials_degree(dim,degree)
        N = poly.shape[0]
...
#        print("C",c,"H",H,"poly",poly,"degree",degree,"wei",alldata_wei)
    ...
    def apply(self, SOL, newdata_par, alldata_par, dim, useless_parameter):
        c, H, poly, degree = SOL
        N = poly.shape[0]
