...
# Define the base name of the folder
base_name = "output_SMU_POLY"

# Get the current directory's contents
dir_contents = os.listdir()

# Filter out directories that match the pattern
matching_folders = [dir for dir in dir_contents if dir.startswith(base_name) and os.path.isdir(dir)]

# Extract numbers from the matching folder names and find the highest one
max_number = -1
for folder in matching_folders:
    try:
        number = int(folder[len(base_name):])
        if number > max_number:
            max_number = number
    except ValueError:
        # In case the folder name does not end in a number, ignore it
        continue

# Create the new folder name with the next number
next_number = max_number + 1
new_folder_name = f"{base_name}{next_number}"
new_folder_name
# basic configuration
...
#     conf.no_parameters, conf.no_observations, kernel="thin_plate_spline")
...
# updater = surrDAMH.surrogates.NNSklearnOngoingUpdater(conf.no_parameters, conf.no_observations, hidden_layer_sizes=(200, 400, 200),
#                                                       activation='tanh', solver='adam', learning_rate_init=1e-4,
#                                                       iterations_batch=100)

# updater = surrDAMH.surrogates.PyTorchNNOngoingUpdater(conf.no_parameters, conf.no_observations, hidden_layer_sizes=(50, 100, 200, 400, 200), learning_rate_init=1e-4,
