...
class Surrogate_apply: # initiated by all SAMPLERs
    ...
    def apply(self, SOL, datapoints):
        ...
        c, alldata_par = SOL
        no_snapshots = alldata_par.shape[0]
        ...
        GS_datapoints=np.matmul(TEMP,c[:-no_polynomials])
        pp = c[-1] + np.matmul(datapoints,c[-self.no_parameters-1:-1])
...
class Surrogate_update: # initiated by COLLECTOR
    def __init__(self, no_parameters, no_observations, initial_iteration=None, no_keep=300, expensive=True, kernel_type=0, solver_tol_exp=-6, solver_type='minres'):
        self.no_parameters = no_parameters
        self.no_observations = no_observations
...
#        print("DEBUG2:", TEMP2.shape, np.linalg.matrix_rank(TEMP2), RHS.shape)
    ...
    #    print("Residual norm:",np.linalg.norm(RES), "RHSnorm:", np.linalg.norm(RHS))
        SOL = [c.copy(), self.processed_par.copy()]
        print("A,b,c,par:",TEMP2.shape, RHS.shape, c.shape, self.processed_par.shape)
