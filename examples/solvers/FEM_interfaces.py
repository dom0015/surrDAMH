...
import examples.solvers.grf_eigenfunctions as grf
...
class FEM:
    ...
    def __init__(self, no_parameters = 5, no_observations = 5, no_configurations = 1, n = 100, quiet = True, tolerance = None, PC = "none", use_deflation = False, deflation_imp = None):    
        ...
        truncate = 100
        self.grf_instance = grf.GRF('examples/solvers/unit50.pckl', truncate=truncate)
        quantiles = np.cumsum(np.ones((no_parameters,))/no_parameters)
        # interfaces_matrix = self.grf_instance.realization_interfaces(np.ones((truncate,)), quantiles=quantiles, nx_new = n, ny_new = n)
        # tmp = interfaces_matrix.reshape((1,n*n))
        # self.interfaces_vector = np.concatenate([tmp,tmp]).reshape((n*n*2,))
