...
def run_COLLECTOR(conf: Configuration, surrogate_updater: Updater, surrogate_delayed_init_data=None):
    ...
    while any(sampler_is_active):  # while at least 1 sampling algorithm is active
        ...
        while True:
            ...
            # print(message_not_received, flush=True)
                ...
                if comm_world.iprobe(source=rank, tag=MPI.ANY_TAG, status=status):
                    ...
                    if tag > TAG_STAGE_FINISHED:
                        ...
                        list_received_snapshots = [
                            np.vstack(
                                (list_received_snapshots[j],
                                 received_snapshot[j])) for j in range(3)]
                    else:
            ...
            # print(all(message_not_received), flush=True)
        ...
        if num_snapshots > 0 and any(sampler_is_active):
            surrogate_updater.add_data(
                list_received_snapshots[0],
                list_received_snapshots[1],
                list_received_snapshots[2])
            if ANALYZE:
                if list_all_snapshots:
                    list_all_snapshots = [
                        np.vstack(
                            (list_all_snapshots[j],
                             list_received_snapshots[j])) for j in range(3)]
                else:
                    list_all_snapshots = list_received_snapshots.copy()
            list_received_snapshots = [
                np.empty(
                    (0, conf.no_parameters)), np.empty(
                    (0, conf.no_observations)), np.empty(
                    (0, 1))]
        cond_init = num_snapshots_used == 0 and num_snapshots >= conf.min_snapshots_initial  # initial surrogate model
