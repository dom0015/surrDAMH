...
**Reference only — not a to-do list.** The author decided 2026-09-17 not to delete dead code
(some of it is implemented for planned use, decision 19, `09_improvement_plan.md` §3). This
report's tables, including "(b) safe to delete", document what is deliberately unused/kept and
why; nothing in it should be acted on without a fresh decision. One known-stale item since
2026-09-17: (c)'s `NeuralNetworkUpdaterMinibatches.save_snapshots` row says only
`sampling_TSX.py` calls `save_snapshots()` and that it's the (now-deleted) Basic updater —
`sampling_TSX.py` was migrated to the Minibatches updater the same day (decision 16), so that
specific claim no longer holds; not corrected in place, per this file's own "reference, not
maintained" status. Inline references below to `04_post_processing.md`/
`05_toy_examples_and_docs.md` point to per-subsystem review notes deleted 2026-09-18
(superseded by `06_findings_consolidated.md`) — historical citations, not live links.
...
| ~~`Samples.pdf_report`~~ | ~~`post_processing.py:828`~~ | **Deleted 2026-09-18** (decision 24, `09_improvement_plan.md` §3) — 0 callers, confirmed safe by `./run_tests.sh unit` (unchanged: 315 passed, 1 skipped). |
| `Samples._load_snapshot_parameters_and_observations`, `Samples.load_snapshots`, `Samples.html_report` | `post_processing/statistics.py`, `loading.py`, `html_report.py` | No current caller anywhere. `html_report` newly caller-less as of WS10 (its one caller, `post_processing_example.py`, was archived as a duplicate). Part of the documented legacy (non-extended) report API — pending an explicit author call on whether it's still wanted (asked 2026-09-18, `10_manual_review_notes.md` §8). |
| `distributions/transformations.py::lognormal_to_normal`, `beta_to_normal` | `:19,54` | **Decided 2026-09-18 (decision 23): keep**, as documented public helpers. Test-only today: exercised by `tests/unit/test_distributions.py:200,207` (round-trip checks). |
...
| 6 | `load_snapshots`, `_load_snapshot_parameters_and_observations`, `html_report` (`pdf_report` already deleted, decision 24) | needs an explicit author decision first (still-documented legacy report path, §c) — no test currently exercises them either way |
