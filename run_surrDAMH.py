...
conf = surrDAMH.Configuration(output_dir=output_dir, no_parameters=3, no_observations=2, use_surrogate=True, initial_sample_type="prior_mean")
...
list_of_components = [Uniform(-4, 4), Uniform(-4, 4), Beta(2, 2), Uniform(3, 5), Lognormal(0, 1)]
...
list_of_stages = []
list_of_stages.append(Stage(algorithm_type="MH", proposal_sd=1.0, max_evaluations=100))
list_of_stages.append(Stage(algorithm_type="DAMH", proposal_sd=None, max_evaluations=100, surrogate_is_updated=True))
list_of_stages.append(Stage(algorithm_type="DAMH", proposal_sd=None, max_evaluations=2000, surrogate_is_updated=False))
...
if rank_world == 0:
    ...
    ensure_dir(os.path.join(output_dir, "post_processing_output"))
    file_path = os.path.join(output_dir, "post_processing_output", "histograms.pdf")
